...
struct AddSavingsSheet: View {
        ...
        NavigationStack {
            VStack(spacing: 20) {
                ...
                LazyVGrid(columns: columns, spacing: 12) {
                    ...
                    keypadSymbolButton(systemName: "delete.left.fill", isAccent: false) {
                        _ = rawDigits.popLast()
                    }
                }
    ...
    @ViewBuilder
    private func keypadSymbolButton(systemName: String, isAccent: Bool = true, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            ZStack {
                Circle()
